...
- xts built in function calculation = `r portfolio_sd_xts_builtin_percentpercent`%
...
# doesn't work and that important as we'll see
port_rolling_sd <- round(na.omit(rollapply(asset_returns_xts, window, 
                           function(x) StdDev(x, weights = w))), 4) * 100
# works
port_rolling_sd <- round(na.omit(rollapply(portfolio_returns_xts_rebalanced_monthly, window, 
                           function(x) StdDev(x))), 4) * 100
```

We now have an `xts` object called `port_rolling_sd` that contains the 6-month rolling standard deviation of returns of SPY.  Keep in mind that the chosen window is important and can affect the results quite a bit. Soon we'll wrap this work to a Shiny app where changing the window and visualizing the results will be easier. 
...
Why did we take that detour? Because we will use `map_df()`, `mutate()` and `select()` when we apply our custom function with the `%>%` operator and that will require a `tibble`. 
