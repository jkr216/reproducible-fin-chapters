...
```
We can also combine the tidyverse with a piece of the tidyquant family and use the `tibbletime` package. 
...
head(rolling_sd_tidy_tibbletime)
```

That works fine, but it's not generalizable since the `RccpRoll` package does not have built-in functions for most statistics we might want to calculate (as we'll see with Kurtosis and Skewness).  

For this reason, `xts` or `tidyquant` is where I generally turn for rolling calculations.
