...
where x is each monthly portfolio return and x-bar is the mean monthly return. For a multi-asset portfolio, the equation is not simply the weighted variances of each asset. Instead, it's the weight squared of each asset times that asset's variance, plus the covariance of each asset times the weights of those assets. 
...
Now, let's use that equation and calculate portfolio standard deviation by-hand.

First, we assign the weights of each asset, based on the `w` vector we created earlier in the Returns section.
Then, we isolate and assign returns of each asset and plug those weights and returns into the equation for portfolio standard deviation.
...
MORE ON WHY WE DID THAT. How does it tie into the dat vis, repro and future work. 
