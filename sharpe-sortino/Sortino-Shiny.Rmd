...
rolling_sortino <-
  apply.rolling(portfolio_returns_xts, window, fun =  "SortinoRatio", MAR = MAR) %>% 
  `colnames<-`("24-rolling")
})
...
sortino_byhand <- 
  ...
  mutate(ratio = mean(returns - MAR)/sqrt(sum(pmin(returns - MAR, 0)^2)/nrow(.)),
         returns_below_MAR = ifelse(returns < MAR, returns, NA),
         returns_above_MAR = ifelse(returns > MAR, returns, NA))
...
### The Sortino Ratio of Your Portfolio

```{r}
valueBoxOutput("approvalBox1")
output$approvalBox1<-renderValueBox({
  valueBox(value = 6, color = "primary")
})
```

### Sortino Ratio of S&P500 in same time period

```{r}

valueBoxOutput("approvalBox2")
output$approvalBox2<-renderValueBox({
  valueBox(value = 5, color = "primary")
})
```
