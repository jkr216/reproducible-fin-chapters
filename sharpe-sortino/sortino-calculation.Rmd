...
Take a quick peek at the rolling 6-month.
...
highchart(type = "stock") %>%
  ...
  hc_add_series(sortino_roll_6_xts, name = "Sortino", color = "blue") %>%
  hc_navigator(enabled = FALSE) %>% 
  hc_scrollbar(enabled = FALSE)
```

The rolling 6-month has so many bizarre spikes, e.g. a reading of in 2016! It nicely emphasizes how we need to be careful with the Sortino Ratio, short time periods and rolling applications.

Let's see how the rolling 12-month compares.

```{r}
MAR <- .008
sortino_roll_12_xts <- rollapply(portfolio_returns_xts_rebalanced_monthly, 24, 
                           function(x) SortinoRatio(x, MAR = MAR))

highchart(type = "stock") %>%
  hc_title(text = "Rolling Sortino") %>%
  hc_add_series(sortino_roll_12_xts, name = "Sortino 12", color = "green") %>%
  hc_navigator(enabled = FALSE) %>% 
  hc_scrollbar(enabled = FALSE)
```
