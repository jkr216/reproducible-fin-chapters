...
### The Tidyverse paradigm: daily prices to monthly returns
...
### The Tidyquant paradigm: daily prices to monthly returns
...
Our second method in the tidyquant world will produce the same output as the previous two - a `tibble` of monthly log returns - but we will also introduce the `tibbletime` package and it's function `as_period`.  
...
Then we add each of our series to the highcharter code flow with `hc_add_series(asset_returns_xts[, symbols[1]], name = symbols[1])`. Notice that we can use our original `symbols` object to refernce the columns. This will allow the code to run should we change to different ticker symbols at the outset.
...
Luckily, we can the `ggplot2` package from the tidyverse and we will operat on our tidy data frame `assets_returns_long`. 
...
Our by-hand method is complete. We now have portfolio returns starting on February 28, 2013 through December 29, 2-17.  Let's confirm we get the same results with the built-in methods.
...
### Portfolio Returns in the tidyverse world.
...
For all the reasons discussed in the introduction, a Shiny application is a flexible, useful and powerful way to share our work.  In this section, we will build a Shiny app to display portfolio returns using the various visualization frameworks above.
...
![sidebar input](input-sidebar.png)
...
![full sidebar](input-sidebar-entire.png)
...
### Portfolio Growth in the Tidyverse world
...
That method again skipped our important step of first finding monthly returns. 

For the sake of fun, let's combine the tidyverse and tidyquant for a code flow still relies on those monthly returns. We will use `mutate()` from the `dplyr` package and create a new column called 'growth with 
...
Here is a snapshot of what the app looks like with the visualizations. 

![portfolio growth app](portfolio-growth-full.png)
